...
class SaveableMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._PICKLER == "dill" and not dill_imported:
            raise dill_import_error
    ...
    def _save(self, filepath: Path, do_overwrite: bool | None = False) -> None:
        if not hasattr(self, "do_overwrite"):
        ...
        for attr in self._DEL_BEFORE_SAVING_ATTRS:
            if hasattr(self, attr):
                skipped_attrs[attr] = self.__dict__.pop(attr)
        ...
        for attr, val in skipped_attrs.items():
