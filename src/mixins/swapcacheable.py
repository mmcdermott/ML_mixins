...
class SwapcacheableMixin:
    ...
    def _init_attrs(self):
        if not hasattr(self, "_cache"):
            self._cache = {"keys": [], "values": []}
        if not hasattr(self, "_cache_size"):
            self._cache_size = 5
        if not hasattr(self, "_front_attrs"):
            self._front_attrs = []
        if not hasattr(self, "_front_cache_key"):
            self._front_cache_key = None
        if not hasattr(self, "_front_cache_idx"):
            self._front_cache_idx = None

    def _set_swapcache_key(self, key: Hashable) -> None:
        self._init_attrs()
        if key == self._front_cache_key:
            return
    ...
    def _update_front_attrs(self):
        ...
        for key, val in self._cache["values"][self._front_cache_idx].items():
